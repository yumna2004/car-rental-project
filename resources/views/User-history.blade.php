    ...
    @if(isset($rentals) && $rentals->count() > 0)
        <div class="rentals-list">
            @foreach($rentals as $rental)
                <div class="rental-card" data-status="{{ $rental->status }}">
                    ...
                    <div class="rental-details">
                        ...
                        <div class="detail-item">
                            ...
                            <div class="detail-content">
                                ...
                                <span>{{ \Carbon\Carbon::parse($rental->end_date)->format('d-m-Y') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="detail-item">
                            ...
                            <div class="detail-content">
                                <label>Duur</label>
                                <span>{{ \Carbon\Carbon::parse($rental->start_date)->diffInDays(\Carbon\Carbon::parse($rental->end_date)) }} dagen</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rental-footer">
                        ...
                        <div class="rental-actions">
                            <a href="{{ route('vehicles.show', $rental->vehicle->id) }}" class="btn btn-secondary btn-small">
                                🚗 Voertuig Bekijken
                            </a>
                            @if($rental->status === 'active' || $rental->status === 'upcoming')
                                <form method="POST" action="{{ route('rentals.cancel', $rental->id) }}" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-small" onclick="return confirm('Weet je zeker dat je deze reservering wilt annuleren?')">
                                        ❌ Annuleren
                                    </button>
                                </form>
                            @endif
